// Limbes — maquette E : les cases, les graines, l’île, l’archipel, en 3D.
// Le même parcours et la même grammaire que la maquette D ; le même stockage, pour voir la même île dans les deux.
// Tout reste sur cet appareil ; rien ne part. Les îles des autres sont inventées.

import { SUBJECTS, QUESTIONS, KEYS, BASE, LEX, HUMANS, MOCK } from '../limbes-d/contenu.js?v=11';
import { graines, quadDe, nomDe, phrasesDe, casesDe, sujetLabel, listeDe, listeGraines, FAMILLES, ESPECES, NOMS } from '../limbes-d/grammaire.js?v=11';
import { nouvelleIle, deriver, resume, archipelInvente, ileInventee, BIOMES, BIOME_IDS, biomeDe } from '../limbes-d/ile.js?v=11';
import { Vue3D, Ilot3D, apercu, disponible } from './monde.js?v=1';
import { lire } from '../limbes-d/lexique.js?v=11';
...
function renderIle() {
  ...
  const tourner = quiet('tourner', () => { vue?.tourner(); note('geste : tourner l’île'); });
  if (mine) {
  ...
  titre = null;
  scene.d = d;
  if (!vue) { wrap.append(el('p', { className: 'sans3d', textContent: 'La 3D n’est pas disponible sur cet appareil. La maquette D montre la même île, en 2D.' })); updateIleLine(); return; }
  vue.attacher(wrap);
...
function renderArchipel() {
  ...
  placerArchipel();
  arch.arrivals = 0;
  if (!vue) { wrap.append(el('p', { className: 'sans3d', textContent: 'La 3D n’est pas disponible sur cet appareil. La maquette D montre l’archipel en 2D.' })); updateArchLine(); return; }
  vue.attacher(wrap);
...
render('q:situ');
requestAnimationFrame(frame);
window.limbesE = { state, get ile() { return ile; }, get iles() { return iles; }, get courant() { return courant; }, get preview() { return preview; }, arch, vie, vue, ilot }; // pour les tests
